# experiments/04_sweep_global_scale.py
...
def collect_eval_arrays(model, loader, device):
    """
    test set 전체를 numpy array로 모아두고,
    FP32 prediction도 한 번만 계산한다.

    이렇게 하면 scale sweep 때 FP32 forward를 반복하지 않아도 된다.
    """
    model.eval()
...
def update_debug_aggregate(debug_global, debug, batch_size):
    """
    debug 값들을 scale 조합별로 모은다.

    - *_min은 전체 batch 중 최소값
    - *_max는 전체 batch 중 최대값
    - *_int32_ok는 전체 batch에서 모두 True여야 True
    - saturation rate는 batch_size 가중 평균
    """
    for key, value in debug.items():
        ...
        if key.endswith("_min"):
            if key not in debug_global:
                debug_global[key] = int(value)
            else:
                debug_global[key] = min(debug_global[key], int(value))

        elif key.endswith("_max"):
            if key not in debug_global:
                debug_global[key] = int(value)
            else:
                debug_global[key] = max(debug_global[key], int(value))

        elif key.endswith("_int32_ok"):
            if key not in debug_global:
                debug_global[key] = bool(value)
            else:
                debug_global[key] = bool(debug_global[key]) and bool(value)
...
def evaluate_one_scale(
    ...
    row = {
        ...
        "fp32_acc": float(100.0 * np.mean(fp32_preds_np == labels_np)),
        "int8_acc": float(int8_acc),
        "acc_drop": float(100.0 * np.mean(fp32_preds_np == labels_np) - int8_acc),
...
def save_rows_csv(rows, path: Path):
    ...
    # row마다 key가 조금 달라도 저장되게 전체 key 합치기
    fieldnames = []
    for row in rows:
        for key in row.keys():
...
def main():
    ...
    # X_SCALE은 입력이 0~1이고 INT8 positive 0~127에 넣으므로
    # 우선 127 이하 위주로 sweep한다.
    x_scales = [32, 48, 64, 80, 96, 112, 127]

    # W_SCALE은 너무 작으면 weight precision 부족,
    # 너무 크면 weight saturation 증가.
    w_scales = [4, 8, 12, 16, 20, 24, 28, 32, 40, 48, 64]
    ...
    print(f"x_scales: {x_scales}")
    print(f"w_scales: {w_scales}")
    ...
    print()
    print("Starting global scale sweep...")
    ...
    csv_path = sweep_dir / f"global_sweep_shift{shift_val}_{bias_mode}.csv"
    save_rows_csv(rows, csv_path)

    # best row 저장
    best = max(rows, key=lambda r: r["int8_acc"])
    best_path = sweep_dir / f"best_global_sweep_shift{shift_val}_{bias_mode}.json"
